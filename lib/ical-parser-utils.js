...
/**
 * Convert common iCalendar parameter values to booleans or numbers.
 * @param {string|number|boolean} value - Raw parameter value.
 * @returns {string|number|boolean} Normalized parameter value.
 */
...
/**
 * Parse semicolon-separated iCalendar parameters into a case-insensitive map.
 * @param {string[]} parameters - Raw parameter segments.
 * @returns {object} Parameter names mapped to normalized values.
 */
...
/**
 * Split a property value on commas that are not escaped with a backslash.
 * @param {string} value - Property value containing comma-separated entries.
 * @returns {string[]} Unescaped property value segments.
 */
...
/**
 * Merge a duplicate UID entry while honoring RFC 5545 SEQUENCE precedence.
 * If an existing entry is a RECURRENCE-ID override and the incoming entry is
 * the base series, merge the base-series fields regardless of whether it uses
 * RRULE, RDATE, or no recurrence rule. This special case preserves the parser's
 * aggregate recurrence state before normal SEQUENCE precedence is applied.
 * @param {object} existingEntry - Previously stored calendar entry.
 * @param {object} incomingEntry - Newly parsed calendar entry.
 * @param {string} uid - Calendar entry UID used in diagnostic messages.
 * @returns {void}
 */
...
/**
 * Apply an RFC 5545 duration to a date, including tolerated vendor formats.
 * The value is parsed token by token after consuming the optional sign and
 * leading "P". This rejects trailing garbage such as "P1DXYZ" or
 * "garbage1D", as well as date-only time units such as the standalone "M" in
 * "P1M". Some real-world vendor formats remain supported, including a "T"
 * before a week count ("PT1W") and multiple units after one "T"
 * ("-PT1W1D2H3M4S").
 * @param {Date} start - Date from which the duration is evaluated.
 * @param {string} durationString - Raw iCalendar duration value.
 * @param {function(Date, number, string): Date} utcAdd - Date arithmetic function.
 * @returns {Date|undefined} Adjusted date, or undefined for invalid durations.
 */
  ...
  if (entry.duration === undefined) {
    entry.end = entry.datetype === 'date-time'
      ...
      : cloneDateWithMeta(
        entry.start,
        entry.start
          ? localDate(entry.start.getFullYear(), entry.start.getMonth(), entry.start.getDate() + 1)
          : entry.start,
      );
    return entry;
  }
...
function buildDateOnlyStamp(date) {
  // Zero-pad the year to 4 digits (RFC 5545 date-fullyear is exactly 4DIGIT).
  // Without this, a year like 935 serializes to "935" and the resulting
  // 7-char stamp (e.g. "9350928") is re-split at fixed 4/2/2 positions by
  // rrule-temporal into "9350-92-8", throwing RangeError: Cannot parse.
...
/**
 * Normalize a local RRULE UNTIL value to the format expected by RRULE engines.
 * @param {string} rruleOnly - RRULE string without DTSTART metadata.
 * @param {Date} startDate - Event start date and timezone metadata.
 * @param {object} tzUtil - Timezone conversion utilities.
 * @returns {string} RRULE with a normalized UNTIL value.
 */
function normalizeRruleUntil(rruleOnly, startDate, tzUtil) {
  ...
  const untilMatch = /UNTIL=(\d{8})(T\d{6})?(Z)?/v.exec(rruleOnly);
  if (!untilMatch) {
    return rruleOnly;
  }
...
/**
 * Create the legacy-compatible recurrence wrapper for a parsed calendar entry.
 * @param {object} entry - Parsed calendar entry with a start date.
 * @param {string} rule - Raw RRULE value.
 * @param {object} dependencies - Temporal, timezone, and wrapper dependencies.
 * @returns {object} Recurrence wrapper exposing the legacy Date-based API.
 */
...
function findVtimezoneInItem(item, tzid) {
  for (const value of Object.values(item)) {
    if (value?.type !== 'VTIMEZONE') {
      continue;
    }
    ...
    const ids = Array.isArray(value.tzid) ? value.tzid : [value.tzid];
...
/**
 * Find a matching VTIMEZONE component in the nested parser stack.
 * @param {object[]} stack - Parser stack containing calendar components.
 * @param {string} [tzid] - Optional timezone identifier to match.
 * @returns {object|undefined} Matching VTIMEZONE component, if present.
 */
function findVtimezoneInStack(stack, tzid) {
  for (const item of (stack || [])) {
    const vtimezone = findVtimezoneInItem(item, tzid);
    if (vtimezone) {
      return vtimezone;
    }
  }
...
/**
 * Create a parser handler for date-valued iCalendar properties.
 * The handler resolves DATE, DATE-TIME, TZID, and VTIMEZONE fallback values
 * while preserving the metadata required by recurrence expansion.
 * @param {object} dependencies - Date attachment and timezone utilities.
 * @param {function(Date, string): Date} dependencies.addTZ - Attach timezone metadata to a date.
 * @param {object} dependencies.tzUtil - Timezone resolution and parsing utilities.
 * @returns {function(string): object} Property handler factory for date-valued fields.
 */
    ...
    return function (value, parameters, curr, stack) {
      ...
      if (isDateOnly(value, parameters)) {
        ...
        if (comps !== null) {
          ...
          // Set all calendar fields together: the Date constructor maps
          // years 0-99 to 1900-1999 (and would normalize 0000-02-29).
          newDate = localDate(Number(comps[1]), Number(comps[2]) - 1, Number(comps[3]));
...
/**
 * Create handlers for component-specific iCalendar properties.
 * @param {object} dependencies - Date handler and UTC duration utilities.
 * @param {function(string): object} dependencies.dateParameter - Create a date property handler.
 * @param {function(Date, number, string): Date} dependencies.utcAdd - Apply UTC-based date arithmetic.
 * @returns {object} Handlers for GEO, CATEGORIES, recurrence, and FREEBUSY.
 */
...
/**
 * Create a handler factory for EXDATE properties.
 * The resulting handler stores date-only and date-time exclusions under keys
 * that support both calendar-date and exact-instance lookups.
 * @param {object} dependencies - Date parser and date-key utilities.
 * @param {function(string): object} dependencies.dateParameter - Create a date property handler.
 * @param {function(Date): string} dependencies.getDateKey - Create a lookup key for an exclusion date.
 * @returns {function(string): object} Property handler factory for EXDATE fields.
 */
