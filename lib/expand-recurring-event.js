import {getDateKey, localDate, utcDateMs} from './date-utils.js';
...
function getFullDaySpanDays(eventData) {
  if (eventData?.start && eventData.end) {
    ...
    const startDay = utcDateMs(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
    const endDay = utcDateMs(endDate.getFullYear(), endDate.getMonth(), endDate.getDate());
...
function calculateEndTime(start, eventData, isFullDay, baseDurationMs) {
  if (isFullDay) {
    ...
    return localDate(start.getFullYear(), start.getMonth(), start.getDate() + daySpan, 0, 0, 0, 0);
  }
...
function adjustSearchRange(from, to, isFullDay, expandOngoing, baseDurationMs) {
  ...
  if (isFullDay) {
    searchFrom = new Date(utcDateMs(from.getFullYear(), from.getMonth(), from.getDate()));
    searchTo = new Date(utcDateMs(to.getFullYear(), to.getMonth(), to.getDate(), 23, 59, 59, 999));
  } else {
    const isMidnight = to.getHours() === 0 && to.getMinutes() === 0 && to.getSeconds() === 0;
    searchFrom = from;
    searchTo = isMidnight
      ? localDate(to.getFullYear(), to.getMonth(), to.getDate(), 23, 59, 59, 999)
      : to;
  }
...
function isInstanceInRange(instance, from, to, expandOngoing) {
  if (instance.isFullDay) {
    const instanceDate = localDate(instance.start.getFullYear(), instance.start.getMonth(), instance.start.getDate());
    const fromDate = localDate(from.getFullYear(), from.getMonth(), from.getDate());
    const toDate = localDate(to.getFullYear(), to.getMonth(), to.getDate());
    const instanceEndDate = localDate(instance.end.getFullYear(), instance.end.getMonth(), instance.end.getDate());
